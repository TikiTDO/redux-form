...
var mergeDeepWithHelper = function mergeDeepWithHelper(
  originalList,
  mergeFunction,
  listToMerge
) {
  if (originalList.size < listToMerge.size) {
    return listToMerge.map(function(value, index) {
      return mergeFunction(originalList.get(index), value)
    })
  } else {
    return originalList.map(function(value, index) {
      return mergeFunction(value, listToMerge.get(index))
    })
  }
}
